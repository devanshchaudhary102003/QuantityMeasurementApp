...
namespace QMAOperationService.Services
{
    public class QuantityMeasurementService : IQuantityMeasurementService
    {
        ...
        public async Task<bool> Compare(QuantityDTO first, QuantityDTO second, int userId)
        {
            Validate(first);
            Validate(second);
            ...
            double savedResult = result ? 1.0 : 2.0;

            await SaveHistory(first, second, "Compare", savedResult, userId);

            return result;
        }

        public async Task<QuantityDTO> Add(QuantityDTO first, QuantityDTO second, int userId)
        {
            Validate(first);
            Validate(second);
            ...
            await SaveHistory(first, second, "Addition", resultBase, userId);

            return CreateBaseResult(first.Category, resultBase);
        }

        public async Task<QuantityDTO> Subtract(QuantityDTO first, QuantityDTO second, int userId)
        {
            Validate(first);
            Validate(second);
            ...
            await SaveHistory(first, second, "Subtraction", resultBase, userId);

            return CreateBaseResult(first.Category, resultBase);
        }

        public async Task<double> Divide(QuantityDTO first, QuantityDTO second, int userId)
        {
            Validate(first);
            Validate(second);
            ...
            await SaveHistory(first, second, "Divide", result, userId);

            return result;
        }

        public async Task<QuantityDTO> Convert(QuantityDTO source, string targetUnit, int userId)
        {
            Validate(source);
            ...
            await SaveHistory(source, second, "Convert", convertedValue, userId);
        ...
        // Calls HistoryService over HTTP
        private async Task SaveHistory(QuantityDTO first, QuantityDTO second, string opr, double result, int userId)
        {
            if (userId == 0) return; // guest — skip

            try
            {
                var historyUrl = _config["Services:HistoryService"] ?? "http://localhost:5003";
                var client = _httpFactory.CreateClient();

                var dto = new SaveHistoryDTO
                {
                    UserId = userId,
                    Value1 = first.Value,
                    Value2 = second.Value,
                ...
                var response = await client.PostAsJsonAsync($"{historyUrl}/api/history", dto);
                if (!response.IsSuccessStatusCode)
                {
                    _logger.LogWarning("HistoryService call failed: {status}", response.StatusCode);
                }
            }
            catch (System.Exception ex)
            {
                // Don't fail the main operation if history save fails
                _logger.LogError(ex, "Failed to save history to HistoryService");
            }
        }
